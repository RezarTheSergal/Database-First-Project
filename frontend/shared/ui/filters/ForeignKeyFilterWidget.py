...
class ForeignKeyFilterWidget(BaseFilterWidget):
    ...
    def _determine_display_column(self) -> str:
        ...
        if self.target_table == None or self.target_column == None:
            raise ValueError()

        display_column = self.target_column  # По умолчанию используем ID колонку

        try:
            response = DatabaseRepository.get_table_columns(self.target_table)
    ...
    def _perform_search(self):
        ...
        if (
            len(query_text) < 2
            or self.target_table == None
            or self.display_column == None
            or self.target_column == None
        ):
            return
        try:
    ...
    def set_value(self, value_id: str, display_text: str = ""):
    ...
    def _load_value_by_id(self, value_id: str):
